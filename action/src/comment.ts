...
function renderDomHintMarkdown(snapshot: SnapshotResult): string {
  ...
  if (snapshot.pageShift) {
    ...
    lines.push(`> ↕️ **Layout shift** — everything below y=${p.belowY} moved ${p.dy > 0 ? 'down' : 'up'} ${Math.abs(p.dy)}px (${p.count} elements together). Look *above* that line for inserted/removed content.`);
  }
...
`;
  ...
  if (missing.length > 0) {
    comment += `> ⚠️ **${missing.length} baseline${missing.length === 1 ? '' : 's'} received no capture this run** — ${missing.slice(0, 5).map(n => `\`${n}\``).join(', ')}${missing.length > 5 ? ', …' : ''}. If a test was removed intentionally, delete its baseline; otherwise coverage just shrank.\n\n`;
  }
...
`;
  ...
  comment += '---\n> ℹ️ Pixel-exact comparison — dynamic content (images, fonts, animations) may cause false positives; the `💡` hint marks likely render noise. Reviewing with an AI agent? [`@testivai/mcp`](https://github.com/mcbuddy/testivai-oss/blob/main/docs/guides/ai-agents.md) serves this report as structured data (`explain_snapshot`).\n\n*Generated by [TestivAI](https://github.com/mcbuddy/testivai-oss)*\n';
